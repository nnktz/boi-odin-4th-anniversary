...
function OnOption8(){
	...
	$Guildid = GetPlayerInfo( -1 , "guildid" )
	$Lingdi = IsHaveGuildZone($Guildid)	
	$item = GetPlayerInfo( -1 , "item" , 44713 )
	$nullitem = GetPlayerInfo( -1 , "nullitem" ,0 )
//	44713	??????
	...
	//????
	if $item < 15
		BC( "dialogbox", "player", -1, "You must have at least 15 Aura of Souljades to exchange for Advanced Construction Material." )	
		return
	endif
	//???
	$result = SubPlayerInfo( -1 ,"item" , 44713 , 15)
	//???????
	if $result == 0
		AddPlayerInfo(-1 , "item" , 59788 , 1)
		BC( "dialogbox", "player", -1, "Successfully exchanged for Advanced Construction Materials." )	
