...
//??20:00-20:30?????						
						...
						BC( "dialogbox", "player", -1, "N/A???~????Rose~" )						
//?????????			
...
// ------------------------------------------????-------------------------------------------------	
				...
				function OnOption2(){
//???????
						$item = GetPlayerInfo( -1, "item", 59202 )
						$nullitem = GetPlayerInfo( -1, "nullitem", 0 )
						$lv = GetPlayerInfo( -1, "level")
//??????
						$qualification = GetPlayerVar( -1, 28 )
...
//??20:00-20:30?????						
						...
						if $Now_Hour > 20
								if $Now_Hour < 24								
										BC( "dialogbox", "player", -1, "I have been given a lot of roses today. Please come back tomorrow between 20:00 and 21:00. " )	
										return	
								endif
						endif				
//??????						
...
//?????????		 ???? lv^2*10*(lv*0.04-0.76)*9
		
						$n1 = $lv * $lv
						$n2 = $n1 * 700
				
						$sub = SubPlayerInfo(-1, "item", 59202, 60 )
						$add = AddPlayerInfo(-1, "item", 59204, 2 )
						$add = AddPlayerInfo(-1, "exp", $n2 )
						BC( "dialogbox", "player", -1, "Great! Thanks. Please come back tomorrow. {#ffff002a=You'll receive 2 Crystal Petals, which you can use to buy Soul Armor from the Armadura Angel.#}" )						
