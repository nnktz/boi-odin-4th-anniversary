	...
	function OnOption0(){
		...
		//------   19:00~21:00   ????--------------
		if $hour < 20
			BC( "dialogbox", "player", -1, "Astral Feast runs from 20:30 to 21:15. Please come back later." )
			return
		endif
		if $hour == 20
			if $minute < 30
		...
		$lv = GetPlayerInfo( -1 , "level" )
		if $lv < 75
			BC("screen","player",-1,"Outer Starway is too dangerous. LV75 or lower players can not enter the map!")
			return
		endif
	...
	function OnOption1(){
		...
		$Task_Time = GetPlayerVar( -1 , 3730 )
		if $lv < 75
			BC("screen","player",-1,"Outer Starway is too dangerous for LV75 or lower players to take the quest.")
			return
		endif
		$info = IsTaskAccept( -1, 4425 )
		if $info == 0
