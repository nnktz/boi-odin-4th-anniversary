...
-----------------------------------------------------------??
		if nIndex == 2 then
	        ...
	        posx = AI:GetPosX()
		     	...
		     	x = math.ceil(posx)
		     	y = math.ceil(posy)
		     	AI:ScreenText("A radiant corona signals the start of the Holy Fortify event in the Grand Rift ("..tostring(x)..","..tostring(y)..")!",2)
--	   r = math.random(1,3)
...
--------------------------------------------------------??????
        if nIndex == 3 then
        AI:ScreenText("Ares has blessed us! Chests full of Fortify Crystals are sprinkled everywhere in the northeast of the Grand Rift.",2)
          posx = AI:GetPosX()
		     	posy = AI:GetPosY()
		     	MonsterID = 32421
