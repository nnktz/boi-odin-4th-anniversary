...
-----------------------------------------------------------??
		if nIndex == 2 then
	        ...
	        posx = AI:GetPosX()
		     	...
		     	x = math.ceil(posx)
		     	y = math.ceil(posy)
		     	AI:ScreenText("A radiant corona signals the star of the Holy Pet event in the Grand Rift ("..tostring(x)..","..tostring(y)..")!",2)
--	   r = math.random(1,3)
...
--------------------------------------------------------??????
        if nIndex == 3 then
        AI:ScreenText("Ares has blessed us! Chests full of Summon Fortify stones are sprinkled everywhere in the southwest of the Grand Rift.",2)
          posx = AI:GetPosX()
		     	posy = AI:GetPosY()
		     	MonsterID = 32422
		     	posx = posx-20
