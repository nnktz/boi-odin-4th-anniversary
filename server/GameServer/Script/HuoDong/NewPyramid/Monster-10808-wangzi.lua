...
--		//49	???-????	????? 576
	...
	function speakshit_A(nAI)
		...
		if randomnuff == 1 then
			AI:Say("Do you really want to obtain the treasure in the Pyramid? Too young, too simple!")
		end
		if randomnuff == 2 then
			AI:Say("I am the God of Doom! You are just my puppet!")
		end
		if randomnuff == 3 then
			AI:Say("I shall be reborn within the flames!")
		end
	...
	function speakshit_B(nAI)
		...
		randomnuff = math.random(1,12)
		if randomnuff == 1 then
			AI:Say("Osiris is not qualified to be my father. His failure is my success. The crown rightfully belongs to me!")
		end
		if randomnuff == 2 then
			AI:Say("Succumb to the seething fires of my anger!")
		end
		if randomnuff == 3 then
			AI:Say("Soothing Flames, surround my body! Be my sword and shield!")
		end
		if randomnuff == 4 then
...
------------------------------------------------------------------------dead
	function Event_OnDead(nAI)
		...
		status=AI:GetArray(1) 
		if status == 1 then
			AI:ScreenText("Prince Arbyss is collecting energy as the Pharaoh Sanctuary appears.",2)
