	...
	//		Modified:2012/06/21
	//		Author:???
	//		TaskName:HD-caihongjiejie-zuanshi3.s
	//		TaskID:????-??????NPC?????
	//
	//****************************************
	function OnRequest(){
	...
	function OnOption0(){
		...
		if $44282 == 1
			...
			SubPlayerInfo( -1 , "item" , 44282 , 1)
			AddPlayerInfo( -1 , "item" , 44283 , 1)
			return
		else
			BC( "screen" , "player" , -1 , "Sorry, you have to open the LV2 barrier first!" )
		endif
	}