	...
	function OnUseItem(){
  	...
  	if $sub != 0
		...
		//?????? ??? 80-60  ??? 40-30  ??? 20-10
		$c = RandomNumber(20,10)
		$b = RandomNumber(40,30)
		$a = RandomNumber(80,60)
		...
		if $buff1 == 0
			...
			if $buff2 == 0
				...
				if $buff3 == 0
					$score = $score + 5
					SetPlayerVar(-1, 4015, $score)
					BC("chat","player",-1,"You lit up the firecrackers and got ",$c," points. Your current points are ",$score)
					BC("screen","player",-1,"You lit up the firecrackers and got ",$c," points. Your current points are ",$score)
				endif				
			endif				
		endif
