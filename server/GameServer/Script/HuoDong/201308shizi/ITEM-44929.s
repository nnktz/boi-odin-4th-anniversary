		...
		if $map == 7
			if $x > 252
				if $x < 277
					if $y > 273
						if $y < 277
							$resule = SubPlayerInfo(-1,"item",44929,1)
							if $resule == 0
								...
								if $quanfu == 3000
									...
									BC("dialogbox","player",-1,"You are the 3000th lucky player to open the Cornucopia. Yourare rewarded a Leo's Pride.")
									DropMonsterItems(-1,90322)
									return
								endif
								if $quanfu == 3500
							...
							BC("screen","player",-1,"The Cornucopia appears in the Bermuda Islands (254, 275) with unbelievable treasure!")
						endif
					else
						BC("screen","player",-1,"The Cornucopia appears in the Bermuda Islands (254, 275) with unbelievable treasure!")
					endif
