	...
	function OnUseItem(){
		...
		if $map != 1
			BC("screen","player",-1,"The cornucopia shows somewhere in the Atlantic Ocean hides unbelievable treasure!")
			return
		endif
		...
		if $map == 1
			if $x > 187
				if $x < 191
					if $y > 83
						if $y < 87
							$resule = SubPlayerInfo(-1,"item",44926,1)
							if $resule == 0
							...
							BC("screen","player",-1,"The Cornucopia appears in the Atlantic Ocean (189, 85) with unbelievable treasure!")
						endif
					else
						BC("screen","player",-1,"The Cornucopia appears in the Atlantic Ocean (189, 85) with unbelievable treasure!")
					endif
