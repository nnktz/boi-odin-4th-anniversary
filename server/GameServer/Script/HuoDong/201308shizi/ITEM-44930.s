		...
		if $map == 13
			if $x > 90
				if $x < 94
					if $y > 150
						if $y < 154
							$resule = SubPlayerInfo(-1,"item",44930,1)
							if $resule == 0
								...
								if $quanfu == 3000
									BC("scrollbar","servergroup",-1,"Player",#name,"You are the lucky 3000th person to open the Cornucopia. You win a Leo's Pride.")
							...
							BC("screen","player",-1,"The Cornucopia shows the coordinates (92, 152) in King's Gorge hides unbelievable treasure!")
						endif
					else
						BC("screen","player",-1,"The Cornucopia shows the coordinates (92, 152) in King's Gorge hides unbelievable treasure!")
					endif
