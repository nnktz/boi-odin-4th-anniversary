	...
	function OnUseItem(){
		...
		if $map != 0
			BC("screen","player",-1,"The cornucopia appears somewhere in Easter Island North with unbelievable treasure!")
			return
		endif
		...
		if $map == 0
			if $x > 111
				if $x < 115
					if $y > 108
						if $y < 112
							$resule = SubPlayerInfo(-1,"item",44920,1)
							if $resule == 0
							...
							BC("screen","player",-1,"The cornucopia appears at (113, 110) in Easter Island North with unbelievable treasure!")
						endif
					else
						BC("screen","player",-1,"The cornucopia appears at (113, 110) in Easter Island North with unbelievable treasure!")
					endif
