	...
	//		Modified:2012/06/21
	//		Author:???
	//		TaskName:HD-caihongjiejie-zuanshi4.s
	//		TaskID:????-??????NPC?????
	//
	function OnRequest(){
	...
	function OnOption0(){
		...
		if $44283 == 1
			...
			SubPlayerInfo( -1 , "item" , 44283 , 1)
			AddPlayerInfo( -1 , "item" , 44284 , 1)
			return
		else
			BC( "screen" , "player" , -1 , "Sorry, you have to open the LV3 barrier first!" )
		endif
	}