		...
		function OnRequest(){
		 $yinji = GetPlayerInfo( -1, "item", 44941)
		 if $yinji == 0
		 		DisableNpcOption(12)
		 		DisableNpcOption(13)
		 		DisableNpcOption(14)
		...
		//??????????
			...
			if $item6 > 0
				...
				if $succeed > 0
					...
					//TellEctypeAdditionalInfo( -1 , "GUID:05623000004" )
				else
		...
		//59831	?????????
			...
			if $item6 > 0
				...
				if $succeed > 0
					...
					//TellEctypeAdditionalInfo( -1 , "GUID:05623000020" )
				else
					BC( "dialogbox", "player", -1, "Try again later." )
					return
				endif
...
//???????????
	...
	//?????????
	function OnOption12(){
 		$count = GetPlayerInfo( -1, "item", 44941 )
 		
 		if $count <= 0
 			BC( "dialogbox", "player", -1, "You don't have Divine Armor Seal!")
 			return
 		endif
 		
 		$result = SubPlayerInfo( -1, "item", 44941, 1 )
 		
 		if $result != 0
			return
		endif
		
		AddPlayerInfo( -1, "item", 44496, 1 )
	}
	//????????
	function OnOption13(){
 		$count = GetPlayerInfo( -1, "item", 44941 )
