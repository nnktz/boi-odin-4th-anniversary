	...
	//		Modified：2008/09/09
	//		Author：祁晶
	//		TaskName：HD-aodingdeenci-32015.s
	//		TaskID：奥丁的恩赐-开宝箱活动-32015号宝箱
	//
	//****************************************
	function OnDead(){
		...
		if $teamplayer1_id != -1
			...
			if $len <= 30
				...
				DropMonsterItems($teamplayer1_id, 60005 )
				//给经验
				//给金钱
				//调用掉落库
			endif
		...
		if $teamplayer2_id != -1
			...
			if $len <= 30
				...
				DropMonsterItems($teamplayer2_id, 60005 )
				//给经验
				//给金钱
				//调用掉落库
			endif
		...
		if $teamplayer3_id != -1
			...
			if $len <= 30
				...
				DropMonsterItems($teamplayer3_id, 60005 )
				//给经验
				//给金钱
				//调用掉落库
			endif
		...
		if $teamplayer4_id != -1
			...
			if $len <= 30
				...
				DropMonsterItems($teamplayer5_id, 60005 )
				//给经验
				//给金钱
				//调用掉落库
			endif
		...
		if $teamplayer6_id != -1
			...
			if $len <= 30
				...
				DropMonsterItems($teamplayer6_id, 60005 )
				//给经验
				//给金钱
				//调用掉落库
			endif
