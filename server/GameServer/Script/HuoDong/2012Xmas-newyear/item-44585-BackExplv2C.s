...
//		BC("chat","player",-1,"fexpori=",$fexpori)
		...
		$playervar = GetPlayerVar(-1,3428)
		if $playervar == $sysmday
			BC("screen","player",-1,"You've already had the Candy (Blessing) today...")
			return
		endif
		$res = SubPlayerInfo(-1,"item",44585,1)
		if $res == 0 
			$addexp = $expori / 1000
			$addexp = $addexp * 175
			$addexp *= 4
			...
			SetPlayerVar(-1,3428,$sysmday)
			BC("screen","player",-1,"You've already had one Returning Player Diamond EXP Candy (Blessing)!")
		endif
