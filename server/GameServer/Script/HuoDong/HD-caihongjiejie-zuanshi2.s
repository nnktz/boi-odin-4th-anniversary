	...
	//		Modified:2012/06/21
	//		Author:???
	//		TaskName:HD-caihongjiejie-zuanshi2.s
	//		TaskID:????-??????NPC?????
	//
	//****************************************
	function OnRequest(){
	...
	function OnOption0(){
		...
		if $44281 == 1
			...
			SubPlayerInfo( -1 , "item" , 44281 , 1)
			AddPlayerInfo( -1 , "item" , 44282 , 1)
			return
		else
			BC( "screen" , "player" , -1 , "Sorry, you have to open the LV1 barrier first!" )
		endif
	}