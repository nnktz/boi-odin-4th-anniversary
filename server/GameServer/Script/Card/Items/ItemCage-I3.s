  //****************************************
	...
	function OnUseItem(){
		...
		if $level < 90
			BC( "screen", "player", -1 , "You have to be LV90 before you can use this Fate Charm!")
			return
		endif
		
		$delor = SubPlayerInfo( -1 , "item", 54046 , 1 )
		if $delor != 0
			return
		endif
		
		SetPlayerVar(-1,231 , 1)
		$gold = RandomNumber( 0, 50 )
		if $gold == 1
			AddPlayerInfo( -1 , "money" , 10000000)
			BC( "screen", "player", -1 , "You received praise from the Goddess of Fate!")
		endif
		
		$lotgold = RandomNumber( 0, 5000 )
		if $lotgold == 1
		...
		$card = RandomNumber( 0, 30 )
		if $card == 1
			$HorN = HaveTarot( -1 , 10272 )
			if $HorN == 0
				AddTarotCard( -1 , 10272 )
			endif
		endif
