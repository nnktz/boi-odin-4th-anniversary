  //****************************************
	...
	function OnUseItem(){
		...
		$meiri = GetPlayerVar(-1,216)
		if $meiri == 1
			BC( "screen", "player", -1 , "You can only use one of each kind of Fate Charm per day!")
			return
		endif
			
		$levelfl = GetFieldLevel(-1)
		if $levelfl < 10
			BC( "screen", "player", -1 , "You have to be Exalted LV10 before you can use this Fate Charm!")
			return
		endif
		
		$delor = SubPlayerInfo( -1 , "item", 54031 , 1 )
		if $delor != 0
			return
		endif
		
		SetPlayerVar(-1,216 , 1)
		$gold = RandomNumber( 0, 30 )
		if $gold == 1
			AddPlayerInfo( -1 , "money" , 10000000)
			BC( "screen", "player", -1 , "You received praise from the Goddess of Fate!")
		endif
		
		$lotgold = RandomNumber( 0, 3000 )
		if $lotgold == 1
		...
		$card = RandomNumber( 0, 45 )
		if $card == 1
			$HorN = HaveTarot( -1 , 10340 )
			if $HorN == 0
				AddTarotCard( -1 , 10340 )
			endif
		endif
