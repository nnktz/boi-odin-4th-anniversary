  //****************************************
	...
	function OnUseItem(){
		...
		if $level < 105
			BC( "screen", "player", -1 , "You have to be LV105 before you can use this Fate Charm!")
			return
		endif
		
		$delor = SubPlayerInfo( -1 , "item", 54041 , 1 )
		if $delor != 0
			return
		endif
		
		SetPlayerVar(-1,226 , 1)
		$gold = RandomNumber( 0, 45 )
		if $gold == 1
			AddPlayerInfo( -1 , "money" , 10000000)
			BC( "screen", "player", -1 , "You received praise from the Goddess of Fate!")
		endif
		
		$lotgold = RandomNumber( 0, 4500 )
		if $lotgold == 1
		...
		$card = RandomNumber( 0, 35 )
		if $card == 1
			$HorN = HaveTarot( -1 , 10295 )
			if $HorN == 0
				AddTarotCard( -1 , 10295 )
			endif
		endif
