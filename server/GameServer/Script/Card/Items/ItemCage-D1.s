  //****************************************
	...
	function OnUseItem(){
		...
		if $level < 45
			BC( "screen", "player", -1 , "You have to be LV45 before you can use this Fatecharm!")
			return
		endif
		
		$delor = SubPlayerInfo( -1 , "item", 54032 , 1 )
		if $delor != 0
			return
		endif
		
		SetPlayerVar(-1,217 , 1)
		$gold = RandomNumber( 0, 75 )
		if $gold == 1
			AddPlayerInfo( -1 , "money" , 10000000)
			BC( "screen", "player", -1 , "You received praise from the Goddess of Fate!")
		endif
		
		$lotgold = RandomNumber( 0, 7500 )
		if $lotgold == 1
		...
		$card = RandomNumber( 0, 20 )
		if $card == 1
			$HorN = HaveTarot( -1 , 10248 )
			if $HorN == 0
				AddTarotCard( -1 , 10248 )
			endif
		endif
