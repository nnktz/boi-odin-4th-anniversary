  //****************************************
	...
	function OnUseItem(){
		...
		if $level < 115
			BC( "screen", "player", -1 , "You have to be LV115 before you can use this Fate Charm!")
			return
		endif
		
		$delor = SubPlayerInfo( -1 , "item", 54042 , 1 )
		if $delor != 0
			return
		endif
		
		SetPlayerVar(-1,227 , 1)
		$gold = RandomNumber( 0, 40 )
		if $gold == 1
			AddPlayerInfo( -1 , "money" , 10000000)
			BC( "screen", "player", -1 , "You received praise from the Goddess of Fate!")
		endif
		
		$lotgold = RandomNumber( 0, 4000 )
		if $lotgold == 1
		...
		$card = RandomNumber( 0, 40 )
		if $card == 1
			$HorN = HaveTarot( -1 , 10303 )
			if $HorN == 0
				AddTarotCard( -1 , 10303 )
			endif
		endif
