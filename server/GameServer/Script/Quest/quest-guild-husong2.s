	...
	function OnTaskDone(){
		...
		if $type == 100
			//??????
			AddPlayerInfo( -1 , "guildmoney" , 24500 )
			BC( "chat" , "guild" , $gid , "(" , #name , " finished the Guild Escort Quest, gaining 24500 Guild Funds." )
		endif
		
		//???????
		if $type == 200
			AddPlayerInfo( -1 , "guildprosperity" , 825 )
			BC( "chat" , "guild" , $gid , "(" , #name , " finished the Guild Escort Quest, gaining 825 Guild Development" )
		endif
		...
		if $week == 0
			if $yunbiao != 0
				AddPlayerInfo( -1 , "guildoffer" , 120 )
			else
