	...
	//		Modified:2009/05/19
	//		Author:???
	//		TaskName:QUEST-guild-2137-2140.s
	//		TaskID:??????
	//
	//****************************************
	...
	function OnTaskDone(){
		...
		if $count > 1
			...
			if $g > $s
				if $type == 1
					$Info1 = SubPlayerInfo( -1 ,"item",12584,-1)
					if $Info1 == 0
						AddPlayerInfo( -1 , "exp",$exp1)
						AddPlayerInfo( -1 , "guildoffer",1)
						AddPlayerInfo( -1 , "item",12586,1)
						BC( "chat", "player", -1, "You have acquired 1 Guild Contribution." )
						SetPlayerVar(-1,3673,1)
					else
						BC( "screen", "player", -1, "You have to acquire a Golden Box of the God of War before you can receive a reward." )
					endif
				endif
				if $type == 2 
					$Info2 = SubPlayerInfo( -1 ,"item",12585,-1)
					if $Info2 == 0
						AddPlayerInfo( -1 , "exp",$exp1) 
						AddPlayerInfo( -1 , "guildoffer",1)
						AddPlayerInfo( -1 , "item",12587,1) 
						BC( "chat", "player", -1, "You have acquired 1 Guild Contribution." )
						SetPlayerVar(-1,3673,1)
					else
						BC( "screen", "player", -1, "You have to acquire a Silver Kettle of the God of War before you can receive a reward." )
					endif
				endif
				SetPlayerVar(-1,3673,1)
			else
				if $type == 1
					$Info3 = SubPlayerInfo( -1 ,"item",12584,-1)
					if $Info3 == 0
				...
				if $type == 2 
					$Info4 = SubPlayerInfo( -1 ,"item",12585,-1)
					if $Info4 == 0
						AddPlayerInfo( -1 , "exp",$exp2)
						AddPlayerInfo( -1 , "guildoffer",1)
						AddPlayerInfo( -1 , "item",12587,1)
			...
			$exp6 = 2 * $c
			if $type == 1
				$Info5 = SubPlayerInfo( -1 ,"item",12584,-1)
				if $Info5 == 0
			...
			if $type == 2
				$Info6 = SubPlayerInfo( -1 ,"item",12585,-1)
				if $Info6 == 0
