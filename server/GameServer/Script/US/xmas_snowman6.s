...
function OnOption0(){
			...
			SetGlobalVar( 206,$n1 )
			if $n1 == 1700000
					BC("scrollbar", "server", -1 , "With your efforts, the sixth snowman is made. Please go to get your gift")
					$n1 = $n1 + 1
					SetGlobalVar( 206,$n1 )
					DeleteNPC( 9016 )
					DeleteNPC( 9024 )
...
function OnOption1(){
			...
			if $a < 6
					BC( "dialogbox", "player", -1, "Just give me six Blessed Snowballs and you can get a gift" )
					return
			endif
			$empty = GetPlayerInfo( -1, "nullitem",0 )
			if $empty < 1
			...
			$sub = SubPlayerInfo(-1,"item",52116,6)


			AddPlayerInfo( -1, "item", 52126, 1 )
			BC( "dialogbox", "player", -1, "Merry Christmas" )
			SetPlayerVar( -1, 3856, $t1 )
