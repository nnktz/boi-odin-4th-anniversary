...
-----------------------------------------------------
	...
	function Event_OnTimer( nAI, nIndex, nCount)
		...
		if nIndex == 1 then--????
			a = AI:GetArray(1)
			if a == 1 then
					AI:Say("The ocean is more powerful than you can imagine.")
			end
			AI:SetArray(1,2)
			AI:SetArray(9,1)
		end--??????
		...
		if nIndex == 2 then--????
			...
			if a == 1 then--????
				...
				if rand < 33 then--??1--start
					...
					AI:UseSkill(2331,2)
					rand = AI:GetRand(100)
					if rand < 40 then--??1??--start
						AI:Say("Tide")
					end--??2??--end
				else--??1--end
					if rand < 80 then--??2--start
						AI:AddSkill(2331,1)
						AI:UseSkill(2331,1)
						rand = AI:GetRand(100)
...
--				AI:ScreenText( "" , 2)
		...
		if nIndex == 4 then--??????
			a = AI:GetArray(4)
			b = AI:GetEctypeVarShort(mapid,45)
			if a == 0 then
				if b == 1 then--?????
					AI:ChangeFaction(11)
					AI:UpdateSeekPath(1)
					AI:ScreenText( "Strong Naga Warrior: You weak little humans want our help? Hah! Not in this life!" , 2)
					AI:SetArray(4,1)
				end
			end
		end--????end			
	...
	function Event_Thinking(nAI)
		...
		-----------------------------------------------------------normal start
			...
			if a == 0 then --?????,????,????
				...
				AI:SetArray(1,1)
		    AI:SetArray(9,0)
		    AI:ScreenText( "Strong Naga Warrior: Who dares to defy the ocean's power?" , 2)
