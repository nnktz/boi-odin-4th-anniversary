...
function Event_OnTimer( nAI, nIndex, nCount)
	...
	if nIndex == 1 then--???1--???
		...
		AI:ScreenText("Tonatiuh: Walking time is over!"..pname,2)
		AI:GetRandTarget()
...
--		sk5max=999
		...
		sk5var=string.rep(jk,math.floor(sk5/sk5max*barmax))
		sk5bar=string.rep(fjk,barmax-math.floor(sk5/sk5max*barmax))
		...
		colorVar=makecolor(sk5/sk5max)
		if timestop > 0 then colorVar="[color=ff0080f0]" end
		msgline=msgline.."\nMagma Volcano: "..colorVar..sk5var..colorEND..colorBar..sk5bar..colorEND
		...
		colorVar=makecolor(sk4/sk4max)
		if timestop > 0 then colorVar="[color=ff0080f0]" end
		msgline=msgline.."\nReflected Shield: "..colorVar..sk4var..colorEND..colorBar..sk4bar..colorEND
		...
		if sk3 == sk3max then
			...
			sk3 = 0
				...
				while i < 9 do
					...
					x3 = math.cos(ARad+0.55)
					y3 = math.sin(ARad+0.55)
					x4 = math.cos(ARad-0.55)
					y4 = math.sin(ARad-0.55)
					...
					AI:SummonMonsterByPos(55383, Xi + 2*i*x3, Yi + 2*i*y3)
					AI:SummonMonsterByPos(55383, Xi + 2*i*x4, Yi + 2*i*y4)
					i = i + 1
				end
		end
		----------------------
...
function Event_OnDead(nAI)
	...
	AI:ScreenText("Tonatiuh: You are so great!"..pname,2)
	AI:DelTimer(1)
	AI:DelTimer(2)
	AI:DelTimer(3)
	AI:DelTimer(4)
...
function makecolor(pctcr)
	...
	sorFL,sorSK,dstFL,dstSK=0.7,0.9,0.5,1
	if pctcr < sorFL then
		pctcr=0
	else
		if pctcr > sorSK then
