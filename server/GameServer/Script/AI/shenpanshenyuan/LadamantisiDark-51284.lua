...
--  Author: ???
--  Class: Lua
--  File Name: LadamantisiDark-51284.lua
--  Remarks: ???????
    ...
    local AI = GetMonsterAI(nAI)
        ...
        if a == 1 then
            if HP < HPMAX * 0.8 then 
                --?????? 
                AI:ScreenText("Dark Latamantis is gathering power!",1)
                AI:Say("Feel my power of darkness!")
...
function Event_OnDead(nAI)
    local AI = GetMonsterAI(nAI)
        AI:ScreenText("Because the darkness disappeared, Light Latamantis lost power.",1)
end  