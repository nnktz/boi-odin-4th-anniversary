------------------------------------------------------
----		Copyright:PERFECT WORLD
----		Modified:2012/08
----		Author:???
----		TaskName:?-BOSS2-Lv1:????-?
----		TaskID:52390_BOSS3_L1.lua
-------------------------------------------------------
--
--	function Event_OnTimer( nAI, nIndex, nCount)
--		local AI = GetMonsterAI(nAI)
--		local mapid = AI:GetMapID()
--		if nIndex == 1 then--????
--			a = AI:GetArray(1)
--			if a == 1 then		
--			end
--			AI:SetArray(1,2)
--			AI:SetArray(9,1)			
--		end--??????
--
--		if nIndex == 2 then--????
--			a = AI:GetArray(2)
--			if a == 1 then--????
--				AI:AddSkill(2385,2)
--				AI:UseSkill(2385,2)
--				AI:Say("GUID:03088000000")
--			end--????end
--		end--????end
--		
--		if nIndex == 3 then--????
--				AI:AddSkill(2385,3)
--				AI:UseSkill(2385,3)
--				AI:ScreenText( "GUID:03088000001" , 2)
--		end--????end
--		
--		if nIndex == 4 then--??1?? ????
--				local PosX = AI:GetPosX()
--				local PosY = AI:GetPosY()
--				a1 = PosX - 5
--				a2 = PosX + 5
--				b1 = PosY - 5
--				b2 = PosY + 5
--				AI:SummonMonsterByPos(52470,a1,PosY,1)
--				AI:SummonMonsterByPos(52470,a2,PosY, 1)
--				AI:SummonMonsterByPos(52470,PosX,b1, 1)
--				AI:SummonMonsterByPos(52470,PosX,b2, 1)
--				AI:SummonMonsterByPos(52470,PosX-3,PosY+3, 1)
--				AI:SummonMonsterByPos(52470,PosX+3,PosY+3, 1)	
--				AI:SummonMonsterByPos(52470,PosX-3,PosY-3, 1)
--				AI:SummonMonsterByPos(52470,PosX+3,PosY-3, 1)				
----				AI:UseSkill(2334,3)
--				AI:ScreenText( "GUID:03088000002" , 2)
--		end--??1??end		
--		
--		if nIndex == 5 then--??3?? ??
--			local PosX = AI:GetPosX()
--			local PosY = AI:GetPosY()
--					for tp = 1 , 4 , 1 do--for
--						a,b = math.random(1,14),math.random(1,18)
--						PosX1 = PosX + a
--						PosX2 = PosX - a
--						PosY1 = PosY + b
--						PosY2 = PosY - b													
--						AI:SummonMonsterByPos(52470,PosX1,PosY1,1)
--						AI:SummonMonsterByPos(52470,PosX1,PosY2,1)
--						AI:SummonMonsterByPos(52470,PosX2,PosY2,1)
--						AI:SummonMonsterByPos(52470,PosX2,PosY2,1)												
--					end--for end
--					AI:ScreenText( "GUID:03088000003" , 2)
--		end--??2??end		
--
--		if nIndex == 6 then--??3?? ??
--				local PosX = AI:GetPosX()
--				local PosY = AI:GetPosY()
--				hinding = AI:GetArray(6)
--				if hinding == 1 then
--					hpn = AI:GetHP()
--					hpmax = AI:GetHPMax()
--					hplose = hpmax - hpn
--					progress = AI:GetArray(3)
--					AI:SetEctypeVarInteger( mapid, 119, hplose )
--					AI:SetEctypeVarShort(mapid,85,PosX)
--					AI:SetEctypeVarShort(mapid,86,PosY)
--					AI:SetEctypeVarShort(mapid,83,1)--?BOSS????  --4???????????
--					AI:SetEctypeVarShort(mapid,88,progress)
--					AI:SetEctypeVarShort(mapid,89,2)
--					AI:PlayAction("specialidle",0,0)
--					AI:ScreenText("GUID:03088000004",1)
--					AI:SetArray(6,2)
--				end
--				if hinding == 2 then
--					a1 = PosX - 7
--					a2 = PosX + 7
--					b1 = PosY - 7
--					b2 = PosY + 7
--					AI:SummonMonsterByPos(52471,a1,b1,135)
--					AI:SummonMonsterByPos(52471,a2,b1,225)
--					AI:SummonMonsterByPos(52471,a1,b2,45)
--					AI:SummonMonsterByPos(52471,a2,b2,315)
--					AI:ScreenText( "GUID:03088000005", 2)
--					AI:SetArray(6,0)
--					AI:Exit()
--				end							
--		end--??3??end				
--		
--		if nIndex == 7 then--??Lv2?? ??Lv2?BOSS
--				local PosX = AI:GetPosX()
--				local PosY = AI:GetPosY()
--				AI:SummonMonsterByPos(52470,PosX-13,PosY-5,1)
--				AI:SummonMonsterByPos(52470,PosX-10,PosY-5,1)
--				AI:SummonMonsterByPos(52470,PosX-7,PosY-5,1)
--				AI:SummonMonsterByPos(52470,PosX-4,PosY-5,1)
--				AI:SummonMonsterByPos(52470,PosX,PosY-5,1)
--				AI:SummonMonsterByPos(52470,PosX+4,PosY-5,1)
--				AI:SummonMonsterByPos(52470,PosX+7,PosY-5,1)
--				AI:SummonMonsterByPos(52470,PosX+10,PosY-5,1)
--				AI:SummonMonsterByPos(52470,PosX+13,PosY-5,1)		
--						
--				AI:SummonMonsterByPos(52470,PosX-13,PosY+4,1)
--				AI:SummonMonsterByPos(52470,PosX-10,PosY+4,1)
--				AI:SummonMonsterByPos(52470,PosX-7,PosY+4,1)
--				AI:SummonMonsterByPos(52470,PosX-4,PosY+4,1)
--				AI:SummonMonsterByPos(52470,PosX,PosY+4,1)
--				AI:SummonMonsterByPos(52470,PosX+4,PosY+4,1)
--				AI:SummonMonsterByPos(52470,PosX+7,PosY+4,1)
--				AI:SummonMonsterByPos(52470,PosX+10,PosY+4,1)
--				AI:SummonMonsterByPos(52470,PosX+13,PosY+4,1)					
--				
--				AI:SummonMonsterByPos(52470,PosX-5,PosY+13,1)
--				AI:SummonMonsterByPos(52470,PosX-5,PosY+10,1)
--				AI:SummonMonsterByPos(52470,PosX-5,PosY+7,1)
--				AI:SummonMonsterByPos(52470,PosX-5,PosY+4,1)
--				AI:SummonMonsterByPos(52470,PosX-5,PosY,1)
--				AI:SummonMonsterByPos(52470,PosX-5,PosY-4,1)
--				AI:SummonMonsterByPos(52470,PosX-5,PosY-7,1)
--				AI:SummonMonsterByPos(52470,PosX-5,PosY-10,1)
--				AI:SummonMonsterByPos(52470,PosX-5,PosY-13,1)						
--				
--				AI:SummonMonsterByPos(52470,PosX+4,PosY+13,1)
--				AI:SummonMonsterByPos(52470,PosX+4,PosY+10,1)
--				AI:SummonMonsterByPos(52470,PosX+4,PosY+7,1)
--				AI:SummonMonsterByPos(52470,PosX+4,PosY+4,1)
--				AI:SummonMonsterByPos(52470,PosX+4,PosY,1)
--				AI:SummonMonsterByPos(52470,PosX+4,PosY-4,1)
--				AI:SummonMonsterByPos(52470,PosX+4,PosY-7,1)
--				AI:SummonMonsterByPos(52470,PosX+4,PosY-10,1)
--				AI:SummonMonsterByPos(52470,PosX+4,PosY-13,1)					
--
--				AI:ScreenText( "GUID:03088000006" , 2)
--		end--??Lv2??end		
--
--		if nIndex == 8 then--?????? ??Lv2?BOSS
--			local PosX = AI:GetPosX()
--			local PosY = AI:GetPosY()
--			Boss1 = AI:GetEctypeVarShort(mapid,60)
--			Boss2 = AI:GetEctypeVarShort(mapid,61)
--			a = AI:GetArray(8)
--			hpn = AI:GetHP()
--			hpmax = AI:GetHPMax()
--			hplose = hpmax - hpn 
--			if a == 1 then
--				if Boss1 == 1 then--BOSS1 DEAD
--					if Boss2 == 1 then--BOSS2 DEAD
--						AI:SetArray(8,2)
--					else
--						AI:SetArray(3,0)
--						AI:RestoreHPInstant(hplose)
--						AI:AddSkill(2382,1)
--						AI:UseSkill(2382,1)
--					end
--				else
--					AI:SetArray(3,0)
--					AI:RestoreHPInstant(hplose)
--					AI:AddSkill(2382,1)
--					AI:UseSkill(2382,1)				
--				end
--			end		
--
--		end--?????? ??Lv2?BOSSend						
--						
--	end--functionEvent_OnTimerEND
--------------------------------------------------------------------------thinking
--	function Event_Thinking(nAI)
--		local AI = GetMonsterAI(nAI)
--		local mapid = AI:GetMapID()
--
--		hpn = AI:GetHP()
--		hpmax = AI:GetHPMax()
--		hplose = hpmax - hpn
--		hppct = hpn/hpmax*100
--		t = AI:HaveAttackTarget()
-- 		a = AI:GetArray(1)
--		n10 = AI:GetEctypeVarShort(mapid,83) --???BOSS?? 10?????
--
--		if t == false then
--			if n10 == 0 then
--				if a > 0 then
--					if hpn == hpmax then
--						CanBeShutDown = AI:GetArray(9)
--						if CanBeShutDown == 1 then				
--					    AI:DelTimer(1)
--					    AI:DelTimer(2)
--					    AI:DelTimer(3)
--							AI:SetArray(1,0)
--							AI:SetArray(2,0)
--							AI:SetArray(3,0)
--							AI:SetArray(4,0)
--							AI:SetArray(5,0)
--							AI:SetArray(6,0)
--							AI:SetArray(7,0)
--							AI:SetArray(8,0)
--							AI:SetArray(9,0)
--							AI:SetEctypeVarInteger( mapid, 119, 0 )--?????0
--						end
--					end
--				end
--			end
--		else
--		-----------------------------------------------------------??BOSS??
--			LimitX = AI:GetPosX()
--			LimitY = AI:GetPosY()
--	    	if LimitY <= 85 	then
--	    		progress = AI:GetArray(3)
--	    		AI:SetEctypeVarShort(mapid,88,progress)
--	    		AI:ScreenText( "GUID:03088000007", 2 )    		
--					AI:SetEctypeVarInteger( mapid, 119, hplose )
--					AI:SetEctypeVarShort(mapid,83,1)--?BOSS????
--					AI:SummonMonsterByPos(52390,75,103,180)
--					AI:SummonMonsterByPos(52441,75,103,180)
--		      AI:Exit()
--	    	end		
--	    	if LimitY >= 125 	then
--	    		progress = AI:GetArray(3)
--	    		AI:SetEctypeVarShort(mapid,88,progress)
--	    		AI:ScreenText( "GUID:03088000008", 2 )
--					AI:SetEctypeVarInteger( mapid, 119, hplose )	
--					AI:SetEctypeVarShort(mapid,83,1)--?BOSS????
--		      AI:SummonMonsterByPos(52390,75,103,180)
--		      AI:SummonMonsterByPos(52441,75,103,180)
--		      AI:Exit()
--	    	end				
--		-----------------------------------------------------------??BOSS??end				
--		-----------------------------------------------------------normal start
-- 			a = AI:GetArray(1)
--			if a == 0 then --?????,????,????
--				hplose = AI:GetEctypeVarInteger( mapid, 119 )--????????,?????????
--				AI:LoseHPInstant( hplose )
--				progress = AI:GetEctypeVarShort(mapid,88)--??BOSS???
--				AI:SetArray(3,progress)
--				AI:SetTimer(1,1000,1)
--				AI:SetArray(1,1)
--		    AI:SetArray(9,0)
--			end
--			a = AI:GetArray(2)
--			if a == 0 then --????,????
--				AI:SetTimer(2,9000,9999)
--				AI:SetArray(2,1)
--			end
--			a = AI:GetArray(8)
--			if a == 0 then
--				AI:SetTimer(8,10000,9999)
--				AI:SetArray(8,1)
--			end
--			a = AI:GetArray(3)
--			if a == 0 then --????
--				if hppct < 95 then
--					AI:SetTimer(4,1000,1)
--					AI:SetArray(3,1)
--				end
--			end
--			if a == 1 then --????
--				if hppct < 90 then
--					AI:SetTimer(5,1000,1)
--					AI:SetArray(3,2)
--				end
--			end			
--			if a == 2 then --????
--				if hppct < 85 then
--					AI:SetTimer(4,1000,1)
--					AI:SetArray(3,3)
--				end
--			end
--			if a == 3 then --????
--				if hppct < 80 then
--					AI:SetTimer(3,1000,1)
--					AI:SetTimer(5,1000,1)
--					AI:SetArray(3,4)
--				end
--			end
--			if a == 4 then --????
--				if hppct < 75 then
--					AI:SetTimer(4,1000,1)
--					AI:SetArray(3,5)
--				end
--			end
--			if a == 5 then --????
--				if hppct < 70 then
--	          AI:SetTimer(7,100,1)
--						AI:SetTimer(5,1000,1)
--						AI:SetArray(3,6)
--						AI:SetArray(7,1)
--				end
--			end
--			if a == 6 then --????
--				if hppct < 65 then
--					AI:SetTimer(4,1000,1)
--					AI:SetArray(3,7)					
--				end
--			end	
--			if a == 7 then --????
--				if hppct < 60 then
--					AI:SetTimer(3,1000,1)
--					AI:SetTimer(5,1000,1)			
--					AI:SetTimer(3,1000,1)		
--					AI:SetArray(3,8)
--				end
--			end
--			if a == 8 then --????
--				if hppct < 55 then
--					hinding = AI:GetArray(6)
--					if hinding == 0 then
--						if n10 == 0 then --??????
--							AI:ChangeFaction(11)
--							AI:SetTimer(6,3000,2)				
--							AI:SetArray(3,9)
--							AI:SetArray(6,1)
--						end
--					end
--				end
--			end
--			if a == 9 then --????
--				if hppct < 50 then
--					AI:SetTimer(5,1000,1)
--					AI:SetArray(3,10)
--				end
--			end			
--			if a == 10 then --????
--				if hppct < 45 then
--					AI:SetTimer(4,1000,1)
--					AI:SetArray(3,11)
--				end
--			end
--			if a == 11 then --????
--				if hppct < 40 then
--					AI:SetTimer(7,1000,1)
--					AI:SetTimer(5,1000,1)
--					AI:SetTimer(3,1000,1)		
--					AI:SetArray(3,12)
--				end
--			end
--			if a == 12 then --????
--				if hppct < 35 then
--					AI:SetTimer(4,1000,1)
--					AI:SetArray(3,13)
--				end
--			end	
--			if a == 13 then --????
--				if hppct < 30 then	
--					hinding = AI:GetArray(6)
--					if hinding == 0 then		
--						if n10 == 0 then --??????			
--							AI:ChangeFaction(11)				
--							AI:SetTimer(6,3000,2)				
--							AI:SetArray(3,14)
--							AI:SetArray(6,1)
--						end
--					end						
--				end
--			end
--			if a == 14 then --????
--				if hppct < 25 then
--					AI:SetTimer(5,1000,1)
--					AI:SetArray(3,15)
--				end
--			end			
--			if a == 15 then --????
--				if hppct < 20 then
--					AI:SetTimer(7,1000,1)
--					AI:SetTimer(4,1000,1)
--					AI:SetTimer(3,1000,1)		
--					AI:SetArray(3,16)
--				end
--			end
--			if a == 16 then --????
--				if hppct < 15 then
--					AI:SetTimer(5,1000,1)
--					AI:SetArray(3,17)
--				end
--			end
--			if a == 17 then --????
--				if hppct < 10 then
--					AI:SetTimer(4,1000,1)
--					AI:SetTimer(3,1000,1)			
--					AI:SetArray(3,18)
--				end
--			end										
--			----------------------------------------------------------normal end
--		end --- t end
--	end--functionEvent_ThinkingEND
-------------------------------------------------------------------------------	thinking end
--			
--	function Event_ArriveSeekPos(nAI, MapID, PosX, PosY)
--		local AI = GetMonsterAI(nAI)
--		local mapid = AI:GetMapID()
--
--		if (( PosX == 75) and ( PosY == 103 )) then	
--			AI:SetTimer(8,3200,2)--??????,?Lv2??
--		end		
--	end	
--	
--	function Event_UpdateSeekPos(nAI,nIndex)
--		local AI = GetMonsterAI(nAI)
--		local mapid = AI:GetMapID()
--				
--		if nIndex == 1 then
--			AI:AddSeekPos(mapid,75,103)
--		end
--	end
--			
--	function Event_OnDead(nAI)
--		local AI = GetMonsterAI(nAI)
--		local mapid = AI:GetMapID()
--
--
--		--AI:ScreenText("",2)
--		AI:Say("GUID:03088000009")
--		AI:DelTimer(1)
--		AI:DelTimer(2)
--		AI:DelTimer(3)
--		AI:DelTimer(4)
--		AI:DelTimer(5)
--		AI:DelTimer(6)
--		AI:DelTimer(7)		
--		AI:DelTimer(9)
--		AI:SetEctypeVarInteger( mapid, 119, 0 )--??BOSS???????,?BOSS?????????0
--
--	end