function Event_Thinking(nAI)
    ...
    if t == false then
        ...
        if mode == 1 then
        	c = AI:GetArray(3)
        	if c == 0 then
        		AI:SummonMonsterByPos(52230, 85.2, 169.3)
        		AI:AddSkill(2134,1)
            AI:UseSkill(2134,1)
            AI:SetArray(3,1)
        	end
        ...
        if HP <= HPMAX * 0.5 then
        	g = AI:GetArray(7)
        	if g == 0 then
        		var = AI:GetEctypeVarShort(Mapid, 0)
    				if var == 0 then
    					AI:SummonMonsterByPos(52231, 91.3, 173.6)
    				end
    				if var == 1 then
    					AI:SummonMonsterByPos(52259, 91.3, 173.6)
    				end
    				if var == 2 then
    					AI:SummonMonsterByPos(52279, 91.3, 173.6)
    				end
    				AI:AddSkill(2134,2)
            AI:UseSkill(2134,2)
...
function Event_OnTimer(nAI,nIndex, nCount)
    ...
    if nIndex == 1 then
    		...
    		if mode == 1 then
    			AI:SetEctypeVarShort(Mapid, 4, 2)
    			AI:SetArray(4,0)
    		end
    		if mode == 2 then
    			AI:SetEctypeVarShort(Mapid, 4, 3)
    			AI:SetArray(5,0)
    		end
    		if mode == 3 then
    			AI:SetEctypeVarShort(Mapid, 4, 1)
