 ...
 //****************************  
	...
	$ectype_id = GetPlayerInfo(-1,"mapid")
		...
		$count_one = GetEctypeVar($ectype_id,82) 
		if $rand == 5 
			if $count_total <= 3
				if $count_one < 1
					BC( "screen", "map", $ectype_id, "A crystal shower is approaching Quartz Grotto. Darkshadow Crystals will soon land in the Middle of the Crystal Nexus!")
					$mon1 = 32535
		...
		//		AddMonsterByFloat( $mon1 , 1, $ectype_id, 187, 116, 1 ,0 ,180 )
					...
					SetEctypeVar($ectype_id,82,$count_one)
				endif
			endif
		endif
	//----------------------------------------------------------------------------
