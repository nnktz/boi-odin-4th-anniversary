 ...
 //****************************  
	...
	$ectype_id = GetPlayerInfo(-1,"mapid")
		...
		$count_one = GetEctypeVar($ectype_id,78) 
		if $rand == 5 
			if $count_total <= 3
				if $count_one < 1
					BC( "screen", "map", $ectype_id, "A crystal shower is approaching Quartz Grotto. Darkshadow Crystals will soon land in the Land of Darkness far to the South!")
					$mon1 = 32535
		...
		//		AddMonsterByFloat( $mon1 , 1, $ectype_id, 144, 170, 1 ,0 ,180 )
					...
					SetEctypeVar($ectype_id,78,$count_one)
				endif
			endif
		endif
	//----------------------------------------------------------------------------
