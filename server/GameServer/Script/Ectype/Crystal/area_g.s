 ...
 //****************************  
	function EnterArea(){
		...
		$count_one = GetEctypeVar($ectype_id,80) 
		if $rand == 5 
			if $count_total <= 3
				if $count_one < 1
					BC( "screen", "map", $ectype_id, "A crystal shower is approaching Quartz Grotto. Darkshadow Crystals will soon land in the Land of Ice in the East!")
					$mon1 = 32535
		...
		//		AddMonsterByFloat( $mon1 , 1, $ectype_id, 144, 170, 1 ,0 ,180 )
					...
					SetEctypeVar($ectype_id,80,$count_one)
				endif
			endif
		endif
	//----------------------------------------------------------------------------
		$var_line = GetEctypeVar($ectype_id,0)
		$var_sleep = GetEctypeVar($ectype_id,69)  
		if $var_line == 1   
			if  $var_sleep == 0
		...
		//BC( "screen", "map", $ectype_id, "GUID:04010000001")
		//BC( "chat", "player", $ectype_id, "GUID:04010000002")
		$enter_count =  GetEctypeVar($ectype_id,84)
		$enter_count += 1
		SetEctypeVar($ectype_id,84,$enter_count)
	}
	
	function LeaveArea(){
	$ectype_id = GetPlayerInfo(-1,"mapid")
	   SetEctypeVar($ectype_id,64,-1) 
