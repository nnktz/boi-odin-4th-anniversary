 ...
 //****************************  
	...
	$ectype_id = GetPlayerInfo(-1,"mapid")
		...
		$count_one = GetEctypeVar($ectype_id,79) 
		if $rand == 5 
			if $count_total <= 3
				if $count_one < 1
					BC( "screen", "map", $ectype_id, "A crystal shower is approaching Quartz Grotto. Darkshadow Crystals will soon land in the neutral zone in the Southeast!")
					$mon1 = 32535
		...
		//		AddMonsterByFloat( $mon1 , 1, $ectype_id, 144, 170, 1 ,0 ,180 )
					...
					SetEctypeVar($ectype_id,79,$count_one)
				endif
			endif
		endif
	//----------------------------------------------------------------------------
	$line = GetEctypeVar($ectype_id,0)
	if $line == 1 
	$count_area = GetEctypeVar($ectype_id,86)
		if $count_area == 0 
			$count_area += 1
			SetEctypeVar($ectype_id,86,$count_area)
			SetEctypeVar($ectype_id,85,1)
		endif
	endif
