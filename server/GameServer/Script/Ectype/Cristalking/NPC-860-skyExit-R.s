...
//BC("screen","player",-1,"time = ",$time)
			if $time < 2100
//				BC("dialogbox","player",-1,"GUID:03953000003")
				return
			endif
			if $time < 2120
//------?????????????process??,??????
				...
				if $process == 1
					...
					BC("chat","guild",$guildid,"Your Guild has boarded The Zephyr!")
				endif
				return
			endif
//------?????????????process??,?????????
...
//				BC("chat","player",-1,"suc=",$suc)
				BC("screen","player",-1,"Eurus' Wasteland is currently in Realm ")
			...
			//????
			if $process == 2
				BC("dialogbox","player",-1,"Your Guild was not one of the top 8 on The Zephyr. You cannot continue on to Eurus' Wasteland.")
				return
			endif
			if $process == 1
				BC("dialogbox","player",-1,"Your Guild was not one of the top 8 on The Zephyr. You cannot continue on to Eurus' Wasteland.")
...
//			BC("dialogbox","player",-1,"GUID:03953000021"$process)
